# Uncertainty and stochastic envrionments in matrix models
#04/18/2017
...
#function isn't doing what we want it to

library(gtools)
x <- rdirichlet(5, c(245,18,8,1)) #1st input = number of elements (replicates), 2nd string of raw counts
print(x)
...
temp2 <- apply(temp,3,damping.ratio) # use apply over the third dimension
 print(temp2) #10 values represent 10 replicates
 str(temp2)
...
# output: matrix of patch transitions, starting from intial vector
#------------------------------------------------- 
...
#unlike deterministic approach from last time 