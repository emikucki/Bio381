---
title: "R_Animation_Package_Presentation"
...
* 3. However, for GIFS and videos you need to download the third party software "ImageMagick" which is not very user friendly....   
...
* Buffon's needle simulation; geometrical probability (buffon.needle)   
* Brownian motion simulation (brownian.motion)   
* Word count of a speech by President Obama (ObamaSpeech)   
* Word counts of a speech by the Chinese President Hu (HuSpeech)   
* Geometric features of pollen grains (pollen)   
* Demonstrate stock prices in animations (price.ani)   

### Simple animations not using preset 'animation' functions
```{r}
...
saveHTML ({
  ...
  for (i in seq_along(idx)) {
    plot(x,y,type='n')
    points(x[seq(idx[i])], y[seq(idx[i])], pch=15, col = "dark green")
    ani.pause()
  }
}, htmlfile = "cos_sin.html", ani.width = 600, ani.height = 600)
```
[Output](simple.plot.html)

###More complicated animation not using preset 'animation' function
```{r}
...
```
####Other uses of the animation package (external links)   
[R Animation Package PDF](animation_R_Package.pdf)   
[Journal of Statistical Software](v53i01.pdf)

####Limitations of the animation package   
* Requires third-party software for most export formats   
* Not particularly straightforward for animating personal data   
* Weird formatting within RMarkdown   
* Glitches in the animations   
